...
def load_data(ticker, start_date, end_date):
    ...
    try:
        ...
        if data is None or (isinstance(data, pd.DataFrame) and data.empty):
            st.error(f"Не удалось загрузить данные для тикера '{ticker}'. Возможно, тикер неверен или нет данных за выбранный период.")
            return None, None, None
        ...
        if 'Adj Close' in data.columns and not data['Adj Close'].isnull().all():
            ...
            price_col = 'Close'
            st.warning("Колонка 'Adj Close' не найдена. Анализ будет выполнен по колонке 'Close', что может быть менее точно (без учета дивидендов).")
        else:
...
def fit_levy_stable_fast(returns_data, max_samples=2000):
    """Быстрая подгонка Леви-стабильного распределения с ограничением выборки."""
    sample_size = min(len(returns_data), max_samples)
    if len(returns_data) > sample_size:
        sample_indices = np.random.choice(len(returns_data), sample_size, replace=False)
        if isinstance(returns_data, pd.Series):
            returns_sample = returns_data.iloc[sample_indices].values
        else:
            returns_sample = returns_data[sample_indices]
    else:
        returns_sample = returns_data.values if isinstance(returns_data, pd.Series) else returns_data
    
    try:
        ls_alpha, ls_beta, ls_loc, ls_scale = levy_stable.fit(
            returns_sample,
            method='mle',
            optimizer='minimize'
        )
        return ls_alpha, ls_beta, ls_loc, ls_scale
    except Exception as e:
        # Fallback на упрощенную оценку
        return 1.8, 0.0, float(np.median(returns_sample)), float(np.std(returns_sample) * 0.7)
...
def plot_distributions_pdf(log_returns, fit_params, ticker):
    ...
    param_data = {
        "Параметр": ["Среднее (μ) / Локация (loc)", "Ст. откл. (σ) / Масштаб (scale)", "Индекс стабильности (α)", "Асимметрия (β)", "Ст. волатильность (ω)", "ARCH (α[1])", "GARCH (β[1])", "Форма (ν)"],
        "Гауссова": [f"{g_mu:.5f}", f"{g_std:.5f}", "-", "-", "-", "-", "-", "-"],
        "Леви-стабильная": [f"{ls_loc:.5f}", f"{ls_scale:.5f}", f"{ls_alpha:.4f}", f"{ls_beta:.4f}", "-", "-", "-", "-"],
        "GARCH(1,1)-t": [f"{garch_params['mu'] / 100:.5f}", f"{last_vol:.5f} (условное)", "-", "-", f"{garch_params['omega']:.5f}", f"{garch_params['alpha[1]']:.4f}", f"{garch_params['beta[1]']:.4f}", f"{nu:.4f}"]
    }
    ...
    ax.hist(log_returns, bins=150, density=True, alpha=0.5, label=f'Эмпирические данные ({ticker})', color='lightblue', edgecolor='blue')
    ...
    ax.plot(x, levy_stable.pdf(x, ls_alpha, ls_beta, ls_loc, ls_scale), 'g-', lw=3, label=f'Леви-стабильное (α={ls_alpha:.2f})')
    
    garch_t_pdf = t.pdf(x, df=nu, loc=garch_params['mu']/100, scale=last_vol)
    ax.plot(x, garch_t_pdf, 'm-', lw=3, label=f'GARCH-t (ν={nu:.2f})')
    
    ax.set_title(f'Сравнение плотностей распределений дневной доходности {ticker}', fontsize=14, fontweight='bold')
    ax.set_xlabel('Логарифмическая доходность', fontsize=12)
    ax.set_ylabel('Плотность вероятности', fontsize=12)
    ax.legend(loc='best', fontsize=11)
    ax.grid(True, linestyle='--', alpha=0.4)
    ax.set_xlim(log_returns.quantile(0.001), log_returns.quantile(0.999))
...
def run_and_plot_var_simulation(fit_params, capital, horizon, confidence, sims=10000):
    ...
    col1.metric(label="VaR (Гаусс)", value=f"${var_g:,.0f}", help=f"С вероятностью {100-confidence:.1f}% убыток НЕ превысит эту сумму.")
    col2.metric(label="VaR (Леви-стабильная)", value=f"${var_ls:,.0f}", delta=f"{((var_ls - var_g) / var_g):.1%}", delta_color="inverse", help="Дельта показывает разницу с Гауссовой моделью.")
    col3.metric(label="VaR (GARCH-t)", value=f"${var_garch:,.0f}", delta=f"{((var_garch - var_g) / var_g):.1%}", delta_color="inverse", help="Дельта показывает разницу с Гауссовой моделью.")
    ...
    ax.hist(losses_ls, bins=bins, density=True, alpha=0.6, label='Леви-стабильная модель', color='green', edgecolor='darkgreen')
    ax.hist(losses_garch, bins=bins, density=True, alpha=0.6, label='GARCH-t модель', color='purple', edgecolor='indigo')
    
    # Добавляем линии VaR
    ax.axvline(var_g, color='darkred', linestyle='--', lw=2.5, label=f'{confidence}% VaR (Гаусс): ${var_g/1e9:.2f}B')
    ax.axvline(var_ls, color='darkgreen', linestyle='--', lw=2.5, label=f'{confidence}% VaR (Леви): ${var_ls/1e9:.2f}B')
    ax.axvline(var_garch, color='indigo', linestyle='--', lw=2.5, label=f'{confidence}% VaR (GARCH-t): ${var_garch/1e9:.2f}B')
    ...
    # Форматирование оси X в миллиардах для читаемости
...
ticker = st.sidebar.text_input("Тикер актива", value="^GSPC", help="Например: ^GSPC (S&P500), AAPL (Apple), BTC-USD (Bitcoin)")
...
if days_diff < 365:
    st.sidebar.warning(f"⚠️ Выбран короткий период ({days_diff} дней). Рекомендуется минимум 1 год для точного анализа.")
...
if st.button("🚀 Запустить анализ", type="primary", use_container_width=True, help="Начать полный анализ рисков"):
    ...
    if data is not None and log_returns is not None:
        ...
        with tab2:
            ...
            var_g, var_ls, var_garch = run_and_plot_var_simulation(fit_params, initial_capital, horizon_days, confidence_level)
        ...
        with conclusion_col1:
            st.success(
                ...
                f"• VaR (99% за {horizon_days} дней): от **${min(var_g, var_garch):,.0f}** до **${max(var_ls):,.0f}**\n\n"
